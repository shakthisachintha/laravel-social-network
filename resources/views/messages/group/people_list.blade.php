...
@else
    ...
    @foreach($user_list as $friend)
        <a href="javascript:;" id="chat-people-list-{{ $friend['user']->id }}" onclick="showGroupChat({{ $friend['user']->id }})" class="friend @if($friend['user']->id == $active_user_id){{ 'active' }}@elseif($friend['new']){{ 'new-message' }}@endif">
